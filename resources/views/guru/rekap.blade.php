                ...
                <div class="form-group">
                    ...
                    <select name="month_filter" id="month_filter" class="form-control" onchange="this.form.submit()">
                        ...
                        @foreach ($months as $key => $month)
                            <option value="{{ $key }}"
                                {{ request()->input('month_filter') == $key ? 'selected' : '' }}>
                                {{ $month }}
                            </option>
                        @endforeach
                    </select>
                        ...
                        @foreach ($exams as $key => $exam)
                            <tr>
                                ...
                                <td class="align-middle text-center">
                                    {{ \Carbon\Carbon::parse($exam->exam_start)->format('d M Y') }}</td>
                                ...
                                @endif
                                <td class="align-middle text-center">
                                    @if ($exam->exam_answer->isEmpty())
                                        <span class="badge bg-warning">Data jawaban tidak tersedia.</span>
                                    @else
                                        @foreach ($exam->exam_answer as $k => $answer)
                                            @if ($answer->is_false === 1)
                                                <button class="btn btn-sm btn-danger mb-1"
                                                    style="width: 36px">{{ $k + 1 }}</button>
                                            @else
